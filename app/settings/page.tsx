...
// ✅ [필살기] 버튼 컴포넌트를 서버에서 아예 안 만들게 격리함
const NoSSRButton = dynamic(() => Promise.resolve(({ 
  active, onClick 
}: { active: boolean; onClick: () => void }) => (
  <button 
    onClick={onClick} 
    className={`w-14 h-8 rounded-full transition-all flex items-center px-1 ${
      active ? 'bg-red-600 justify-end' : 'bg-gray-300 justify-start'
    }`}
  >
      ...
      <main className="max-w-3xl mx-auto px-6 py-16">
        ...
        <section className="space-y-12">
          ...
          <div className="flex items-center justify-between py-4">
            ...
            {mounted && <NoSSRButton active={settings.darkMode} onClick={() => toggleSetting('darkMode')} />}
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {mounted && <NoSSRButton active={settings.marketAlert} onClick={() => toggleSetting('marketAlert')} />}
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {mounted && <NoSSRButton active={settings.newsLetter} onClick={() => toggleSetting('newsLetter')} />}
          </div>
        </section>
